<?php 
    session_start();
    include '../conn.php'; 
    $sql2 = "SELECT * FROM classescontent WHERE class='commerce'";
    ...
    if(isset($_SESSION['useradmin']['status'])) {
        ...
        if(isset($_POST['commerce'])) {
            ...
            //$commerce= count($_FILES['syllabuscommerce']['name']);
            if(empty($pagetitle)) {
               ...
               /* $target_path = "../uploads/";  
                ...
                for( $i=0 ; $i < $commerce ; $i++ ) {
                    
                    //Get the temp file path
                    $tmpFilePath = $_FILES['syllabuscommerce']['tmp_name'][$i];
                    $newFilePath = $target_path  . $_FILES['syllabuscommerce']['name'][$i];
                    $filename = $_FILES['syllabuscommerce']['name'][$i];

                    //Upload the file into the temp dir
                    if(move_uploaded_file($tmpFilePath, $newFilePath)) {
                        //$target_path = $target_path.basename( $_FILES['syllabuscommerce']['name']);   
                    ...
                    $sql = "INSERT INTO syllabus (class, subject, filename, uploaded_at) VALUES ('commerce', 'commerce', '$filename', NOW())";
                ...
                if (mysqli_num_rows($res) < 1 ) {
                    $sql1 = "INSERT INTO classescontent (class, title, description, created_at) VALUES ('commerce', '$pagetitle', '$classdesc', NOW())";
                    ...
                    $sql3= "UPDATE `classescontent` SET `class`='commerce',`title`='$pagetitle',`description`='$classdesc',`created_at`='now()' WHERE id='$id'";
...
?>
    ...
    <div id="content">
        <div id="content-header">
            <div id="breadcrumb"> <a href="index.html" title="Go to Home" class="tip-bottom"><i class="icon-home"></i> Dashboard</a> <a href="#" class="">commerce</a> <a href="#" class="current">commerce Content</a> </div>
            <h1>commerce</h1>
        </div>
        ...
        ?>
            <form class="form-horizontal" action="<?php echo $_SERVER["PHP_SELF"];?>" method="post" name="basic_validate" id="basic_validate" novalidate="novalidate" enctype="multipart/form-data">
                ...
                <!--div class="row-fluid">
                    <div class="widget-box">
                        ...
                        <div class="control-group">
                            ...
                            <div class="controls">
                                <input type="file" name="syllabuscommerce[]" id="file" required multiple>
                            </div>
                        </div>
                    </div>
                </div-->                
                <div class="row-fluid">
                    ...
                    <div class="widget-content">
                        ...
                        <label class="control-label">Description</label>
                            <div class="controls">
                                <textarea class="textarea_editor_commerce span12" rows="6" placeholder="Enter text ..." name="classdesc"><?php echo isset($description) ? $description : '' ?></textarea>
                            </div>
                        </div>
                    </div>
                    </div>
                ...
                <div class="form-actions">
                    <input type="submit" value="Save" class="btn btn-success" name="commerce">
                </div>
            </form>
        </div>
    </div>
        ...
        <script>
            $('.textarea_editor_commerce').wysihtml5();
        </script>
