<?php 
    session_start();
    include '../conn.php'; 
    $sql2 = "SELECT * FROM classescontent WHERE class='icse'";
    ...
    if(isset($_SESSION['useradmin']['status'])) {
        ...
        if(isset($_POST['icse'])) {
            ...
            //$icse= count($_FILES['syllabusicse']['name']);
            if(empty($pagetitle)) {
               ...
               /* $target_path = "../uploads/";  
                ...
                for( $i=0 ; $i < $icse ; $i++ ) {
                    ...
                    $tmpFilePath = $_FILES['syllabusicse']['tmp_name'][$i];
                    $newFilePath = $target_path  . $_FILES['syllabusicse']['name'][$i];
                    $filename = $_FILES['syllabusicse']['name'][$i];

                    //Upload the file into the temp dir
                    if(move_uploaded_file($tmpFilePath, $newFilePath)) {
                        //$target_path = $target_path.basename( $_FILES['syllabusicse']['name']);   
                    ...
                    $sql = "INSERT INTO syllabus (class, subject, filename, uploaded_at) VALUES ('icse', 'icse', '$filename', NOW())";
                ...
                if (mysqli_num_rows($res) < 1 ) {
                    $sql1 = "INSERT INTO classescontent (class, title, description, created_at) VALUES ('icse', '$pagetitle', '$classdesc', NOW())";
                    ...
                    $sql3= "UPDATE `classescontent` SET `class`='icse',`title`='$pagetitle',`description`='$classdesc',`created_at`='now()' WHERE id='$id'";
...
?>
    ...
    <div id="content">
        <div id="content-header">
            <div id="breadcrumb"> <a href="index.html" title="Go to Home" class="tip-bottom"><i class="icon-home"></i> Dashboard</a> <a href="#" class="">Add Syllabus <a href="#" class="current">ICSE</a> </div>
            <h1>ICSE</h1>
        </div>
        ...
        ?>
            <form class="form-horizontal" action="<?php echo $_SERVER["PHP_SELF"];?>" method="post" name="basic_validate" id="basic_validate" novalidate="novalidate" enctype="multipart/form-data">
                ...
                <!--div class="row-fluid">
                    <div class="widget-box">
                        ...
                        <div class="control-group">
                            ...
                            <div class="controls">
                                <input type="file" name="syllabusicse[]" id="file" required multiple>
                            </div>
                        </div>
                    </div>
                </div-->                
                <div class="row-fluid">
                    ...
                    <div class="widget-content">
                        ...
                        <label class="control-label">Description</label>
                            <div class="controls">
                                <textarea class="textarea_editor_icse span12" rows="6" placeholder="Enter text ..." name="classdesc"><?php echo isset($description) ? $description : '' ?></textarea>
                            </div>
                        </div>
                    </div>
                    </div>
                ...
                <div class="form-actions">
                    <input type="submit" value="Save" class="btn btn-success" name="icse">
                </div>
            </form>
        </div>
    </div>
        ...
        <script>
            $('.textarea_editor_icse').wysihtml5();
        </script>
