<?php 
    session_start();
    include '../conn.php'; 
    $sql2 = "SELECT * FROM classescontent WHERE class='jeeadvance'";
        ...
        if(isset($_POST['jeeadvance'])) {
            ...
            if(empty($pagetitle)) {
               ...
               /* $target_path = "../uploads/";  
                ...
                for( $i=0 ; $i < $jeeadvance ; $i++ ) {
                    
                    //Get the temp file path
                    $tmpFilePath = $_FILES['syllabusjeeadvance']['tmp_name'][$i];
                    $newFilePath = $target_path  . $_FILES['syllabusjeeadvance']['name'][$i];
                    $filename = $_FILES['syllabusjeeadvance']['name'][$i];

                    //Upload the file into the temp dir
                    if(move_uploaded_file($tmpFilePath, $newFilePath)) {
                        //$target_path = $target_path.basename( $_FILES['syllabusjeeadvance']['name']);   
                    ...
                    $sql = "INSERT INTO syllabus (class, subject, filename, uploaded_at) VALUES ('jeeadvance', 'jeeadvance', '$filename', NOW())";
                ...
                if (mysqli_num_rows($res) < 1 ) {
                    $sql1 = "INSERT INTO classescontent (class, title, description, created_at) VALUES ('jeeadvance', '$pagetitle', '$classdesc', NOW())";
                    ...
                    $sql3= "UPDATE `classescontent` SET `class`='jeeadvance',`title`='$pagetitle',`description`='$classdesc',`created_at`='now()' WHERE id='$id'";
...
?>
    ...
    <div id="content">
        <div id="content-header">
            <div id="breadcrumb"> <a href="#" title="Go to Home" class="tip-bottom"><i class="icon-home"></i> Dashboard</a> <a href="#" class="">JEE-Advance</a> <a href="#" class="current">JEE-Advance Content</a> </div>
            <h1>JEE-Advance Content</h1>
        </div>
        ...
        ?>
            <form class="form-horizontal" action="<?php echo $_SERVER["PHP_SELF"];?>" method="post" name="basic_validate" id="basic_validate" novalidate="novalidate" enctype="multipart/form-data">
                ...
                <!--div class="row-fluid">
                    <div class="widget-box">
                        <div class="widget-title"> <span class="icon"> <i class="icon-info-sign"></i> </span>
                            <h5>jeeadvance Files</h5>
                        </div>
                        <div class="control-group">
                            <label class="control-label">File</label>
                            <div class="controls">
                                <input type="file" name="syllabusjeeadvance[]" id="file" required multiple>
                            </div>
                        </div>
                    </div>
                </div-->                
                <div class="row-fluid">
                    ...
                    <div class="widget-content">
                        ...
                        <label class="control-label">Description</label>
                            <div class="controls">
                                <textarea class="textarea_editor_jeeadvance span12" rows="6" placeholder="Enter text ..." name="classdesc"><?php echo isset($description) ? $description : '' ?></textarea>
                            </div>
                        </div>
                    </div>
                    </div>
                ...
                <div class="form-actions">
                    <input type="submit" value="Save" class="btn btn-success" name="jeeadvance">
                </div>
            </form>
        </div>
    </div>
        ...
        <script>
            $('.textarea_editor_jeeadvance').wysihtml5();
        </script>
